...
def compute_bayesian_tables(disc:          DiscretizationResult,
    ...
    for ci, cls in enumerate(class_labels):
        ...
        p_h_and_f[ci] = node.health_dist.get(cls, 0) / max(node.n_users, 1) ############################ CHANGED
...
def compute_healthy_range(disc:   DiscretizationResult,
    ...
    if n_healthy > 0:
        ...
        ################################################################## This area was also changed.

        min_healthy_bin = int(healthy_bins.min())
        max_healthy_bin = int(healthy_bins.max())

# bin edges: left edge of bin i = b_raw_min + i * delta_b
#            right edge of bin i = b_raw_min + (i+1) * delta_b
        b_min = float(disc.b_raw_min + min_healthy_bin * disc.delta_b)
        b_max = float(disc.b_raw_min + (max_healthy_bin + 1) * disc.delta_b)
        n_kf  = max_healthy_bin - min_healthy_bin + 1 
