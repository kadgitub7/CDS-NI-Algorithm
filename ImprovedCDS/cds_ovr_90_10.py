"""CDS OVR — 90/10 Train/Test Split with preprocessing (run 10 times, report best).
...
def run_split(data, labels, train_frac=0.9, seed=42):
    n = data.shape[0]
...
def print_report(results, label="90/10 Split"):
...
if __name__ == "__main__":
    ...
    for run in range(10):
        ...
        results = run_split(data, labels, train_frac=0.9, seed=seed)
        acc = sum(r[3] for r in results) / len(results) * 100
        if acc > best_acc:
            best_acc = acc
    ...
    print_report(best_results, label="90/10 Split + Preprocessing (Best of 10 runs)")
