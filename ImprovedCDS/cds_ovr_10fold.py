"""CDS OVR — 10-Fold Cross-Validation with preprocessing (run 10 times, report best).
...
def run_10fold(data, labels, seed=42):
    n = data.shape[0]
    ...
    for fold_i, test_idx in enumerate(folds):
        ...
        print(f"  Fold {fold_i+1}/10 done  running_acc={acc:.1f}%  {time.perf_counter()-t0:.1f}s", flush=True)

    return results


def print_report(results, label="10-Fold CV"):
    all_cls = sorted(set(r[1] for r in results) | set(r[2] for r in results))
    n = len(results)
    correct = sum(r[3] for r in results)
    print(f"\n{'='*70}")
    print(f"CDS One-vs-Rest {label} — {n} users")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    for run in range(10):
        ...
        results = run_10fold(data, labels, seed=seed)
        acc = sum(r[3] for r in results) / len(results) * 100
    ...
    print_report(best_results, label="10-Fold CV + Preprocessing (Best of 10 runs)")
