"""CDS-OVR Sweep v2 — informed by round 1 results.

Round 1 findings:
  - Dual split (AFEvidence+CVrr) best structure (77.7% mean)
  - Normalization helps but z-score is outlier-sensitive
  - Threshold 2.0-2.2 is sweet spot
  - 30-40 FPC is optimal range
  - Sensitivity is the bottleneck (detecting abnormals)

New strategies explore:
  G: Rank-transform preprocessing (robust, preserves order for binning)
  H: Triple split (3 features) for even finer subgroups
  I: Optimized D with Winsorized normalization (clip 5th/95th)
  J: Score-ratio tuning — adjust RATIO_EPS and Fisher weighting
  K: Asymmetric against_scale — very low for abnormal, moderate for healthy
  L: Combined best: triple split + rank + optimized params
"""
import numpy as np
import pickle
import sys
...
STRATEGIES = {
    # G: Rank-transform (percentile) normalization + dual split
    # Rationale: Rank transform is perfectly suited to CDS binning because
    # it maps all features to [0,1] uniformly, making supervised bin edges
    # more stable and comparable across features. No outlier sensitivity.
    "G_rank_dual": {
        "TREE_SPLIT_FEAT": [0, 7],    # AFEvidence + CVrr
        "TREE_MAX_LVL": 1,
        "U_MIN": 200,
        "LAPLACE_ALPHA": 1.0,
        "CORR_THRESHOLD": 0.8,
        "FEATURES_PER_CLASS": 35,
        "MAX_BINS": 7,
        "HEALTHY_WEIGHT": 1.0,
        "HEALTHY_BAR_CAP": 4.0,
        "CLASS_THRESHOLD": 2.0,
        ...
        "AGAINST_SCALE": {1: 0.8, 2: 0.65},
        "RATIO_EPS": 0.1,
        "PREPROCESS": "rank",
    },

    # H: Triple split (AFEvidence + CVrr + IrrEvidence) — 8 subpopulations
    # Rationale: D's success shows more splits help. 3 features at level 1
    # creates up to 8 leaf nodes, each getting a tailored model.
    # With 8482 records, each node still has ~1000+ patients.
    "H_triple_rank": {
        "TREE_SPLIT_FEAT": [0, 7, 2],  # AFEvidence + CVrr + IrrEvidence
        "TREE_MAX_LVL": 1,
        "U_MIN": 150,                   # allow smaller nodes (8 splits)
        "LAPLACE_ALPHA": 1.0,
        "CORR_THRESHOLD": 0.8,
    ...
    # I: Winsorized z-score (5th/95th clip THEN normalize) + optimized D
    # Rationale: Pure z-score is outlier sensitive; clipping first then
    # normalizing gives clean Gaussian-like distributions. Combined with
    # D's proven dual-split structure and tuned threshold.
    "I_winsor_dual": {
        "TREE_SPLIT_FEAT": [0, 7],
        "TREE_MAX_LVL": 1,
        "U_MIN": 200,
        ...
        "HEALTHY_WEIGHT": 0.95,
        "HEALTHY_BAR_CAP": 3.5,
        "CLASS_THRESHOLD": 1.9,         # slightly lower to help sensitivity
        "MIN_SUPPORT": {1: 3, 2: 2},
        "CONF_SUPPORT": {1: 10, 2: 8},
        "AGAINST_SCALE": {1: 0.8, 2: 0.55},  # less penalty for abnormal
        "RATIO_EPS": 0.08,
        "PREPROCESS": "winsor_zscore",
    },

    # J: Score ratio tuning — lower RATIO_EPS + higher Fisher weight floor
    # Rationale: RATIO_EPS=0.1 might be too generous a floor, making weak
    # classes appear to have more evidence than they do. Lowering it makes
    # the ratio more sensitive to actual signal. Also test with rank preprocessing.
    "J_ratio_tuned": {
        "TREE_SPLIT_FEAT": [0, 7],
        "TREE_MAX_LVL": 1,
        "U_MIN": 200,
        ...
        "MIN_SUPPORT": {1: 3, 2: 3},
        "CONF_SUPPORT": {1: 10, 2: 10},
        "AGAINST_SCALE": {1: 0.75, 2: 0.7},
        "RATIO_EPS": 0.05,              # lower floor = sharper ratio
        "PREPROCESS": "rank",
    },

    # K: Asymmetric tuning — heavily favor abnormal detection
    # Rationale: For clinical screening, missing an arrhythmia is worse
    # than a false alarm. Very low against_scale for abnormal (0.4) means
    # conflicting evidence barely counts against an AF prediction.
    # Combined with triple split for subgroup specificity.
    "K_asym_triple": {
        "TREE_SPLIT_FEAT": [0, 7, 2],
        "TREE_MAX_LVL": 1,
        "U_MIN": 150,
        "LAPLACE_ALPHA": 0.8,
        "CORR_THRESHOLD": 0.8,
        "FEATURES_PER_CLASS": 35,
        "MAX_BINS": 7,
        "HEALTHY_WEIGHT": 0.85,          # less healthy advantage
        "HEALTHY_BAR_CAP": 3.0,
        "CLASS_THRESHOLD": 1.7,          # low barrier for abnormal
        "MIN_SUPPORT": {1: 3, 2: 2},
        "CONF_SUPPORT": {1: 12, 2: 7},
        "AGAINST_SCALE": {1: 0.85, 2: 0.4},  # barely penalize abnormal
        "RATIO_EPS": 0.08,
        "PREPROCESS": "winsor_zscore",
    },

    # L: Combined best — triple split + rank + carefully tuned middle ground
    # Rationale: Take the winning structural choices (triple split, rank)
    # and find the middle ground on thresholds that maximizes overall accuracy.
    "L_combined_best": {
        "TREE_SPLIT_FEAT": [0, 7, 2],
        "TREE_MAX_LVL": 1,
        "U_MIN": 150,
        ...
        "CLASS_THRESHOLD": 1.9,
        "MIN_SUPPORT": {1: 3, 2: 2},
        "CONF_SUPPORT": {1: 10, 2: 9},
        "AGAINST_SCALE": {1: 0.8, 2: 0.55},
        "RATIO_EPS": 0.08,
        "PREPROCESS": "rank",
    },
}
...
def preprocess(data, cfg):
    ...
    if mode == "rank":
        # Rank-transform: convert each feature to percentile [0, 1]
        for c in range(N_FEAT):
            col = out[:, c]
            valid = ~np.isnan(col)
            if valid.sum() > 2:
                ...
                out[valid, c] = ranks / len(ranks)
        return out

    elif mode == "winsor_zscore":
        # Clip at 5th/95th percentile then z-score
        for c in range(N_FEAT):
            col = out[:, c]
            valid = ~np.isnan(col)
            if valid.sum() > 10:
                lo = np.nanpercentile(col, 5)
                hi = np.nanpercentile(col, 95)
                out[valid, c] = np.clip(col[valid], lo, hi)
                mu = np.nanmean(out[:, c])
                sd = np.nanstd(out[:, c])
                if sd > 1e-10:
                    out[valid, c] = (out[valid, c] - mu) / sd
        return out

    return out


# ─── CDS core (parameterized) ───
        ...
        for nd in lvl_nodes[lvl]:
            for a in retained:
                ...
                if shift >= 0:
                    ...
                    n_against += 1
                n_used += 1
    return af_for, af_against, n_used, n_for, n_against
...
if __name__ == "__main__":
    strategy_name = sys.argv[1] if len(sys.argv) > 1 else "G_rank_dual"
    ...
    print(f"  RATIO_EPS:   {cfg['RATIO_EPS']}")
