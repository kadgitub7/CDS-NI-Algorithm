"""Find features where supervised uses 2-4 bins (not 1, not 6) with good separation."""
import sys, os
...
for cls in [2, 3, 4, 5, 6, 9, 10]:
    ...
    for fi in range(m.N_FEAT):
        ...
        sv_nb = len(sv_edges) - 1
        if sv_nb < 2 or sv_nb >= max_nb: continue
        ...
        results.append({
            ...
            'ew_nb': max_nb, 'sv_nb': sv_nb,
            'ew_pur': ew_pur, 'sv_pur': sv_pur,
            'sep': sep, 'gain': sv_pur - ew_pur,
            'sv_edges': [f'{e:.1f}' for e in sv_edges],
        })

results.sort(key=lambda x: (x['gain'], x['sep']), reverse=True)
print(f"{'Cls':>3} {'Feat':>5} {'n_t':>4} {'EW':>2}/{' SV':>3} {'EW_p':>5} {'SV_p':>5} {'Gain':>5} {'Sep':>5}  SV edges")
print("-" * 90)
for r in results[:25]:
    print(f"{r['cls']:>3} {r['feat']:>5} {r['nt']:>4} "
          f"{r['ew_nb']:>2} / {r['sv_nb']:>2}  "
          f"{r['ew_pur']:>5.2f} {r['sv_pur']:>5.2f} "
          f"{r['gain']:>+5.2f} {r['sep']:>5.2f}  {r['sv_edges']}")
