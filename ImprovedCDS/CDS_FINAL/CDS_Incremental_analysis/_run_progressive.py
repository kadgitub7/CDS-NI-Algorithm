"""Run progressive build-up variants (P01-P10) with all 10 seeds and merge into CSVs."""
import os
import sys
import subprocess
import csv
...
TIMEOUT = 7200  # 2 hours per variant (binning ones are slow)
ALL_SEEDS = [13, 34, 55, 69, 76, 7, 21, 42, 58, 83]


def find_progressive_files():
    return sorted(DIR.glob("P[0-9][0-9]_*.py"))
...
def main():
    workers = int(sys.argv[sys.argv.index('--workers') + 1]) if '--workers' in sys.argv else min(os.cpu_count() or 4, 3)

    files = find_progressive_files()

    # Allow filtering to specific variants
    if '--filter' in sys.argv:
        pattern = sys.argv[sys.argv.index('--filter') + 1]
        files = [f for f in files if pattern.lower() in f.stem.lower()]

    if not files:
        print("No progressive variant files (P##_*.py) found!")
        return

    print(f"Running {len(files)} progressive variants with {len(ALL_SEEDS)} seeds each")
    print(f"Workers: {workers}, timeout: {TIMEOUT}s each")
    print(f"{'='*70}", flush=True)

    # Read existing per-seed data, excluding progressive reruns
    existing_rows = []
    seed_csv = DIR / 'results_per_seed.csv'
    if seed_csv.exists():
        with open(seed_csv, encoding='utf-8') as f:
            existing_rows = list(csv.DictReader(f))
        prog_names = {f.stem for f in files}
        existing_rows = [r for r in existing_rows if r['variant'] not in prog_names]
    ...
    print(f"\n{'='*70}")
    print(f"Done in {wall:.0f}s. Added {len(new_rows)} progressive rows")
    print(f"{'='*70}")

    print(f"\n{'Variant':<45s} {'10-fold':>8s} {'90/10m':>8s} {'Binary':>8s} {'60/40':>8s}")
    print(f"{'-'*45} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
    prog_names = {f.stem for f in find_progressive_files()}
    for v in variant_order:
        if v not in prog_names and v not in ('01_ovr_baseline', '99_final'):
            continue
        d = by_variant[v]
