"""Run synergy variants (S01-S09) with all 10 seeds and merge into CSVs."""
import os
import sys
import subprocess
import csv
...
def find_synergy_files():
    return sorted(DIR.glob("S[0-9][0-9]_*.py"))
...
def main():
    workers = int(sys.argv[sys.argv.index('--workers') + 1]) if '--workers' in sys.argv else min(os.cpu_count() or 4, 6)

    files = find_synergy_files()
    if not files:
        print("No synergy variant files (S##_*.py) found!")
        return

    print(f"Running {len(files)} synergy variants with {len(ALL_SEEDS)} seeds each")
    ...
    if seed_csv.exists():
        ...
        existing_variants = {r['variant'] for r in existing_rows}
        synergy_names = {f.stem for f in files}
        existing_rows = [r for r in existing_rows if r['variant'] not in synergy_names]
        print(f"Loaded {len(existing_rows)} existing per-seed rows (excluding synergy reruns)")
    ...
    print(f"\n{'='*70}")
    print(f"Done in {wall:.0f}s. Added {len(new_rows)} synergy rows")
    print(f"Total rows in per-seed CSV: {len(all_rows)}")
    print(f"{'='*70}")

    print(f"\n{'Variant':<45s} {'10-fold':>8s} {'90/10m':>8s} {'Binary':>8s} {'60/40':>8s} {'Seeds':>6s}")
    print(f"{'-'*45} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*6}")
    synergy_names = {f.stem for f in files}
    for v in variant_order:
        if v not in synergy_names:
            continue
        d = by_variant[v]
        ...
        print(f"{v:<45s} {cv:>8s} {m9:>8s} {b9:>8s} {m6:>8s} {n_seeds:>6d}")
    print(f"{'='*70}")
