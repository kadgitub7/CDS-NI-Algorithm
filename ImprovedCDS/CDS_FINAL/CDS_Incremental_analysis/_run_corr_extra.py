"""Run R06 and R07 correlation variants with all 10 seeds (separate from main batch)."""
import os
import sys
import subprocess
import csv
...
def main():
    files = [DIR / "R06_corr_090.py", DIR / "R07_corr_095.py"]
    files = [f for f in files if f.exists()]

    print(f"Running {len(files)} extra correlation variants with {len(ALL_SEEDS)} seeds each")
    print(f"Workers: {len(files)}", flush=True)

    new_rows = []
    completed = 0
    t_start = time.time()

    with ProcessPoolExecutor(max_workers=len(files)) as pool:
    ...
    # Append to per-seed CSV
    seed_csv = DIR / 'results_per_seed.csv'
    existing_rows = []
    if seed_csv.exists():
        with open(seed_csv, encoding='utf-8') as f:
            existing_rows = list(csv.DictReader(f))
    extra_names = {f.stem for f in files}
    existing_rows = [r for r in existing_rows if r['variant'] not in extra_names]
    ...
    metric_names = {'10fold_cv': '10-fold CV', '9010_multi': '90/10 multiclass',
                    '9010_binary': '90/10 binary', '6040_multi': '60/40 multiclass'}
    summary_fields = ['variant', '10-fold CV', '90/10 multiclass', '90/10 binary', '60/40 multiclass']
    ...
    for v in extra_names:
        if v in by_variant:
            ...
            print(f"  {v}: 10fold={cv} 9010m={m9} binary={b9} 6040={m6}")
