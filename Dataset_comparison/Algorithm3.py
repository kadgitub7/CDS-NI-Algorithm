...
# WFDB-compatible generalized version
# ─────────────────────────────────────────────────────────────────────────────
...
def _healthy_class_ids_from_schema(schema: Optional[DatasetSchema]) -> FrozenSet[int]:
    if schema is None or schema.healthy_class_ids is None:
        return frozenset()
    return frozenset(schema.healthy_class_ids)


def _is_healthy_class(class_id: int, healthy_class_ids: FrozenSet[int]) -> bool:
    return class_id in healthy_class_ids


def _disease_classes_for_node(node: TreeNode, healthy_class_ids: FrozenSet[int]) -> List[int]:
    class_dist = _node_class_distribution(node)
    return sorted(
        [h for h, cnt in class_dist.items() if h not in healthy_class_ids and cnt > 0]
    )


def _node_feature_name(node: TreeNode, feature_idx: int, tree: Optional[DecisionTree] = None) -> str:
    if tree is not None and hasattr(tree, "schema") and tree.schema is not None:
        return tree.schema.get_feature_name(feature_idx)
    if hasattr(node, "feature_indices"):
...
class Algorithm3Output:
    ...
    def retained_for_node_disease(
        ...
        acts = [
            a for a in self.refined_actions
...
def validate_fa0_constraint(
    retained: List[ExecutiveActionEntry],
    healthy_class_ids: FrozenSet[int],
) -> List[str]:
    """
    ...
    violations: List[str] = []
    for a in retained:
        if a.disease_class in healthy_class_ids:
            violations.append(
                f"FA=0 violation: action for healthy class "
                f"h={a.disease_class} retained at node {a.node_id!r} feat {a.feature_idx}"
            )
    return violations
...
def run_all_validations(
    ...
    all_ok = True
    separator = "─" * 70
    healthy_class_ids = _healthy_class_ids_from_schema(getattr(tree, "schema", None))
    ...
    nodes_processed = list(
        {e.node_id for e in alg3_output.refined_actions}
    ...
    fa0_issues = validate_fa0_constraint(alg3_output.refined_actions, healthy_class_ids)
    if fa0_issues:
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 5 – CORE HELPER: RAW VALUE LOOKUP
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 6 – CORE HELPER: RANGE CHECK
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 7 – CORE ALGORITHM 3 LOGIC (ONE NODE)
# ─────────────────────────────────────────────────────────────────────────────
...
def _refine_one_node(
    ...
    disease_classes: List[int] = _disease_classes_for_node(node, healthy_class_ids)
        ...
        for h_rank, action in enumerate(h_actions_sorted):
            ...
            if model_entry is None:
                ...
                log_records.append(RefinementRecord(
                    ...
                    feature_idx=o,
                    feature_name=o_name,
                    action_weight_orig=action.action_weight,
                    action_weight_final=0.0,
                    b_min=float("nan"),
                    b_max=float("nan"),
                    n_users_node=n,
                    n_users_outside=0,
                    ...
                    buffer_after=buffer,
                    was_retained=False,
                    prune_reason="no perceptor model",
                    h_rank=h_rank,
            ...
            log_records.append(RefinementRecord(
                ...
                feature_idx=o,
                feature_name=o_name,
                action_weight_orig=orig_weight_lookup,
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 8 – NODE SUMMARY COMPUTATION
# ─────────────────────────────────────────────────────────────────────────────
...
def _compute_node_summary(
    ...
    disease_classes = _disease_classes_for_node(node, healthy_class_ids)
...
def run_algorithm3(
    ...
    for nid in nodes_to_process:
        ...
        node = tree.all_nodes[nid]
        log.info(f"\n  Processing node {nid!r}  m={node.focus_level}  n_users={node.n_users}")
    ...
    if n_total_before > 0:
        log.info(f"  Retention rate    : {n_total_retained / n_total_before * 100:.1f}%")
    log.info("=" * 70)
    return output
...
def print_algorithm3_summary(output: Algorithm3Output) -> None:
    ...
    print(f"\n  Per-node breakdown:")
    print(f"  {'node_id':30s} {'m':3} {'before':8} {'retain':8} {'rate%':7} {'flag_u':7} {'buf':5}")
    print(f"  {'─'*65}")
    for nid, s in sorted(output.node_summaries.items()):
        print(
            f"  {nid:30s} {s.focus_level:3d} {s.n_actions_before:8d} "
...
def print_refinement_detail(
    ...
    print(
        f"  {'h':3} {'rank':5} {'feat':4} {'name':20} {'r_orig':8} "
...
def print_per_disease_retained(
    ...
    for h in sorted(by_h):
        ...
        print(
            f"    {'rank':4} {'feat':4} {'name':25} {'r_{o|h}':8} "
...
def print_coverage_analysis(
    ...
    for h in sorted(coverage[node_id]):
        ...
        print(f"    {'feat':4} {'name':22} {'sens%':7} {'flagged':9} {'total':7} {'r':8}")
        print(f"    {'─'*60}")
        sorted_feats = sorted(
            feats.items(),
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 12 – FIDELITY ASSESSMENT
# ─────────────────────────────────────────────────────────────────────────────
...
[PAPER -> Code] Mapping:
  ...
  Line 1  for disease classes h do
            -> disease_classes = non-healthy classes present in the node
            -> for h in disease_classes:
            FIDELITY: ✓ Exact in generalized form.
  ...
  Lines 4-9  for o in features; for u in users: if outside -> newinf_u = 1
            -> for action in h_actions_sorted:
                  raw_vals = data[node.user_indices, o]
                  flag_mask = valid & ((raw_vals > b_max) | (raw_vals < b_min))
                  newinf |= flag_mask
...
─────────────────────────────
  [AMBIG-1] Scope of buffer/newinf initialisation.
    ...
    Legacy-compatible mode: reset_per_h=False for global accumulation.
    Optional mode: reset_per_h=True for per-disease pruning.
  ...
  [AMBIG-3] Strict vs non-strict inequalities in line 6.
    ...
    Note: values at the boundary are considered within range.
...
WFDB GENERALIZATION NOTES
──────────────────────────
  1. Healthy-vs-disease is no longer hardcoded to UCI class ids.
  2. Healthy classes are obtained from the dataset schema.
  3. Under the requested WFDB assumption, "N" (Normal beat) is healthy and
     all other record-level classes are unhealthy.
  4. Feature names and feature dimensionality are schema-driven, not fixed.
""")
    print(sep)


# ─────────────────────────────────────────────────────────────────────────────
# SECTION 13 – MAIN EXECUTION (WFDB-COMPATIBLE)
# ─────────────────────────────────────────────────────────────────────────────
...
def main(
    ...
    if run_full:
        ...
        nodes_filter = [nid for nid in key_nodes if nid in tree.all_nodes]
    ...
    healthy_class_ids = _healthy_class_ids_from_schema(schema)
    disease_candidates = sorted([c for c in set(labels.tolist()) if c not in healthy_class_ids])
