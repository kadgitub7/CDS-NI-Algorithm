...
def main(
    ...
    Parameters
    ----------
    wfdb_path    : path to WFDB records directory.
    ...
    dat_files = glob.glob(os.path.join(wfdb_path, "*.dat"))
    record_names = [os.path.splitext(os.path.basename(f))[0] for f in dat_files]
    log.info(f"  Found {len(record_names)} records in {wfdb_path}")
...
if __name__ == "__main__":
    import sys
    wfdb_path = sys.argv[1] if len(sys.argv) > 1 else "./wfdb"
