...
def main(
    ...
    Parameters
    ----------
    wfdb_path      : path to directory containing WFDB records.
...
def get_wfdb_path(default_dirname: str = "wfdb") -> Path:
    """
    Find the WFDB dataset directory in a cross-platform way.

    Search order:
      1. Command-line argument
      2. Current working directory / wfdb
      3. Same directory as this script / wfdb
      4. data/wfdb beside script
    """
    if len(sys.argv) > 1:
        p = Path(sys.argv[1]).expanduser().resolve()
        if p.exists():
            return p
        raise FileNotFoundError(f"WFDB path not found: {p}")

    cwd_path = Path.cwd() / default_dirname
    if cwd_path.exists():
        return cwd_path.resolve()

    script_dir = Path(__file__).parent
    script_path = script_dir / default_dirname
    if script_path.exists():
        return script_path.resolve()

    data_path = script_dir / "data" / default_dirname
    if data_path.exists():
        return data_path.resolve()

    raise FileNotFoundError(
        f"Could not locate WFDB directory '{default_dirname}'. "
...
if __name__ == "__main__":
    import sys as _sys
    default_path = str(Path(__file__).parent / "data" / "wfdb")
    path = _sys.argv[1] if len(_sys.argv) > 1 else default_path
