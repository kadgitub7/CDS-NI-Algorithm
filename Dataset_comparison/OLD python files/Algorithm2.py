...
def load_wfdb_dataset(
    ...
    wfdb_database : str
        ...
        This must be a database name recognized by `wfdb`, not a filesystem path.
    ann_ext : str
    ...
    -------
    data : np.ndarray
        Shape (N_records, D) tabular feature matrix.
    labels : np.ndarray
        Shape (N_records,) integer labels derived from annotation symbols.

    Notes
    -----
    This converts waveform records into fixed-size tabular features so they can
    be consumed by Algorithm 1 and Algorithm 2.

    IMPORTANT
    ---------
    Algorithm 1 requires a tabular feature matrix of shape (N, N_FEATURES).
    Therefore this loader enforces that the extracted feature dimension equals
    `N_FEATURES`. If the WFDB-derived features do not match, a ValueError is raised.
    """
...
def _has_valid_binary_protected_feature(data: np.ndarray, idx: int) -> bool:
    if data is None or idx < 0 or idx >= data.shape[1]:
        return False
    col = data[:, idx]
...
def compute_bayesian_tables(
    ...
    for ci, cls in enumerate(class_labels):
        p_h_and_f[ci] = node.health_dist.get(cls, 0) / N_total if N_total > 0 else 0.0
...
def compute_executive_actions(
    ...
    if ENABLE_REWEIGHING and data is not None and _has_valid_binary_protected_feature(data, SEX_FEATURE_INDEX):
...
def run_algorithm2(
    ...
    if ENABLE_REWEIGHING and _has_valid_binary_protected_feature(data, SEX_FEATURE_INDEX):
        ...
        log.warning(
            ...
            f"SEX_FEATURE_INDEX={SEX_FEATURE_INDEX}. Skipping fairness/gender analysis."
        )
...
def run_all_validations(output: Algorithm2Output, data: np.ndarray, labels: np.ndarray) -> None:
    print("\n" + "=" * 72)
...
def main(
    ...
    wfdb_database : str
        WFDB / PhysioNet database identifier, e.g. "mitdb".
        This is not a local file path.
    ann_ext : str
        Annotation extension to use when extracting record labels.
    run_full : bool
        If True, process all tree nodes. If False, process root + key level-2 nodes.
    max_records : Optional[int]
        If set, only load the first N records from the WFDB database.
    """
    import logging as _logging
    _logging.getLogger("CDS.Alg1").setLevel(_logging.WARNING)

    log.info("Step 1: Load WFDB dataset")
    data, labels = load_wfdb_dataset(
    ...
    disease_candidates = [h for h in disease_candidates if h != HEALTHY_CLASS][:4]
