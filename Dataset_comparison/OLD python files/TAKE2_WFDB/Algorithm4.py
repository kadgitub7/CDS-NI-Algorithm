...
def compute_N_FEATURES_from_WFDB(WFDB, first_record: str, n_stats_per_channel: int = 4) -> int:
    """
    Compute number of features from a WFDB record.
    """
...
def run_algorithm4(
    ...
    for current_focus in range(1, max_focus_level + 1):
        ...
        if decision in (HealthDecision.HEALTHY, HealthDecision.SCREENING):
            break
...
def print_action_ranking_summary(output: Algorithm4Output, top_k: int = 10) -> None:
    sep = "=" * 65
    print(f"\n{sep}")
    print("ALGORITHM 4 ACTION RANKING SUMMARY")
    print(sep)
    ...
    for rec in output.records:
        for entry in rec.af_trace:
            if not entry.is_nan:
                feature_counts[entry.feature_idx] += 1

    sorted_features = sorted(feature_counts.items(), key=lambda x: x[1], reverse=True)
    print(f"  Top {top_k} most frequently tested features:")
    for idx, (feat_idx, count) in enumerate(sorted_features[:top_k], start=1):
        fname = FEATURE_NAMES.get(feat_idx, f"feat_{feat_idx}")
        print(f"    #{idx:2d}: feat={feat_idx:3d} ({fname})  tested {count} times")
...
================================================================================
[... truncated in this snippet for brevity ...]
"""
    print(mapping)
...
def load_database_WFDB(WFDB, record_names: List[str]) -> Tuple[np.ndarray, np.ndarray]:
    """
    Extract per-user features and class labels from a WFDB database.

    Parameters:
        WFDB          : the wfdb module (already imported)
        record_names  : list of WFDB record names (e.g., ["100", "101", "102"])

    Returns:
        data   : (N_users, N_features)
        labels : (N_users,)
    """
    ...
    # Class mapping: Normal = 1, diseased = 2..n
    class_map = {"N": 1}
    next_class = 2
    ...
    for rec_name in record_names:
        ...
        try:
            ...
            rhythm = ann.aux_note[0].strip() if len(ann.aux_note) > 0 else "UNKNOWN"
        except:
            rhythm = "UNKNOWN"

        # Map class label
        if rhythm not in class_map:
            class_map[rhythm] = next_class
            next_class += 1

        label = class_map[rhythm]

        # Extract simple statistical features per channel
        # (Replace with your own feature extractor if needed)
    ...
    labels = np.array(class_labels, dtype=int)
...
def main(
    ...
    log.info("=" * 65)
    log.info("CDS ALGORITHM 4: USER-HEALTH PREDICTION PIPELINE (WFDB MODE)")
    log.info("=" * 65)
    ...
    data, labels = load_database_WFDB(WFDB, record_names)
    ...
    # ---------------------------------------------------------
    # Step 2: Set feature metadata (N_FEATURES, FEATURE_NAMES)
    # ---------------------------------------------------------
    # N_FEATURES is computed from the first record; no import-time side effects.
    ...
    # ---------------------------------------------------------
    # Step 3: Build models for targeted test cases (optional)
    # ---------------------------------------------------------
    if run_tests:
    ...
    # ---------------------------------------------------------
    # Step 4: LOOCV (Algorithm 4)
    # ---------------------------------------------------------
    if run_loocv_flag:
        log.info(
            "Step 3: Algorithm 4 – LOOCV with per-fold retraining "
        ...
        if false_alarm_users:
            print(f"\n{'=' * 65}")
            print(
                f"FALSE ALARM USERS: {len(false_alarm_users)} healthy users predicted UNHEALTHY"
            )
            print(f"{'=' * 65}")
...
if __name__ == "__main__":
    ...
    # You define the record list explicitly
    record_names = ["100", "101", "102", "103", "104"]  # example

    n = int(_sys.argv[1]) if len(_sys.argv) > 1 else None
