...
def compute_healthy_range(disc:   DiscretizationResult,
    ...
    if n_healthy > 0:
        ...
        #
        # b_min/b_max are the raw min/max of training healthy users' values.
        # This is the direct interpretation of Eq. 5.  FA = 0 on training data
        # is guaranteed by construction (all healthy training values are in range).
        # FA = 0 on test data (LOOCV) is achieved by Algorithm 3 pruning actions
        # for features where only boundary-case users fall outside the range — as
        # the user noted: "the action that chooses this boundary value in a user
        # who is healthy but just outside the healthy range has a probability that
        # is very low and filtered out."
        # [PAPER] Line 13: N_kf = (b_max - b_min) / ΔB
...
def compute_executive_actions(disc:          DiscretizationResult,
    ...
    # ── [PAPER] r_{o|h}: fraction of disease-class users outside [b_min, b_max] ─
    # r_{o|h} = P(OB < b_min | h) + P(OB > b_max | h)
    # [PAPER] Uses the same strict inequalities as the alarm check in Alg 3/4.
    # We compute this directly from raw values so it is consistent with the
    # actual range check performed at prediction time.
    ...
    for cls in bayes.class_labels:
        ...
        # ── [PAPER] Line 19: r_{o|h} via RAW values ─────────────────────────
        # Fraction of disease-class users whose raw feature value is strictly
        # outside the healthy range — exactly what Algorithms 3 and 4 test.
        disease_mask  = (labels_valid == cls)
        disease_vals  = disc._raw_values_valid[disease_mask]
        valid_disease = disease_vals[~np.isnan(disease_vals)]

        if len(valid_disease) == 0:
            continue

        n_below = int((valid_disease < b_min).sum())
        n_above = int((valid_disease > b_max).sum())
        n_disease_total = len(valid_disease)

        p_below = n_below / n_disease_total
        p_above = n_above / n_disease_total
        r_o_h   = p_below + p_above

        # ── [PAPER] Line 18 condition ──────────────────────────────────────
        if r_o_h > 0 and p_h_f > 0:
