...
-----------------------
  ...
  [ENGR]   – engineering choice with explicit justification.
  [AMBIG]  – paper is ambiguous; chosen interpretation is documented.

MATHEMATICAL ANALYSIS
---------------------
The pseudocode iterates over:
  1.  h ∈ {2 … H}                      (disease classes)
  2.  o ∈ O_m^{k,f} sorted by r_{o|h}↓  (features, best-first)
  3.  u ∈ U_m^k(f_m^k)                  (users in the current node)

  For each (h, o) pair:
    • We check whether each user's raw value for feature o falls outside the
      healthy range [b_min, b_max] extracted by Algorithm 2 (Eq. 5).
    • newinf_u = 1  if  BD_m^k(o, u) > b_max  OR  BD_m^k(o, u) < b_min
...
KEY INTERPRETATION NOTE [AMBIG]
---------------------------------
  The pseudocode initialises buffer and newinf ONCE, before the outer h loop.
  This means both variables accumulate globally across ALL disease classes AND
  all features.  Once a user is added to newinf for one (h, o) pair, they stay
  flagged for all subsequent pairs.

  Consequence: the algorithm implements a GLOBAL greedy set-cover over users,
...
INTEGRATION
-----------
Algorithm 3 is a downstream consumer of Algorithm 2.  It uses:
  • Algorithm2Output.executive_library       (action weights to sort / prune)
  • Algorithm2Output.perceptor_library       (healthy ranges for range checks)
  • Raw data matrix  (BD_m^k(o, u) = data[global_user_idx, feature_idx])
  • TreeNode.user_indices                    (which users belong to the node)
  • TreeNode.health_dist                     (which disease classes are present)

Algorithm 3 does NOT re-run Algorithms 1 or 2.

OUTPUT
------
  Algorithm3Output
    .refined_actions        – the pruned executive library (retained entries)
    .removed_actions        – entries pruned by Algorithm 3
    .refinement_log         – per-(node, h, o) decision records
    .summary                – per-node statistics
================================================================================
"""
...
class RefinementRecord:
    ...
    focus_level        : m.
    disease_class      : h ∈ {2 … H}.
    feature_idx        : o – the feature / sensor being evaluated.
    feature_name       : human-readable feature name.
    action_weight_orig : r_{o|h} as computed by Algorithm 2 (before pruning).
    action_weight_final: r_{o|h} after Algorithm 3 (0 if pruned).
    b_min              : lower healthy-range boundary for this (node, feature).
    b_max              : upper healthy-range boundary for this (node, feature).
    n_users_node       : total users in this node (|U_m^k(f)|).
    n_users_outside    : number of users in node with value outside [b_min, b_max]
                         for feature o  *at this point in the algorithm* (NOT
                         cumulative – this is the incremental count for feature o).
    s_cumulative       : s = Σ newinf after processing this feature (cumulative
                         across all h and o seen so far in this Algorithm 3 run).
    buffer_before      : Buffer value BEFORE the line-15 update.
    buffer_after       : Buffer value AFTER line-15 update (= s_cumulative).
    was_retained       : True if action was kept; False if pruned.
    prune_reason       : short explanation if pruned.
    h_rank             : position of this feature in the r-sorted list for h.
    h_order            : global processing order index across all (h, o) pairs.
    reset_mode         : 'global' or 'per_h' (see AMBIG note above).
    """
    node_id:             str
    focus_level:         int
    disease_class:       int
    ...
    b_max:               float
    n_users_node:        int
    n_users_outside:     int         # INCREMENTAL for this feature alone
    s_cumulative:        int         # CUMULATIVE sum(newinf) after this step
    buffer_before:       int
    buffer_after:        int
    was_retained:        bool
    prune_reason:        str
...
class NodeRefinementSummary:
    ...
    [INFER] Produced once per tree node, after all (h, o) decisions for that
    node have been made.
    ...
    node_id           : tree node identifier.
    focus_level       : m.
    n_disease_classes : number of disease classes present in this node (h ≥ 2).
    n_actions_before  : actions in executive library before Algorithm 3 for this node.
    n_actions_retained: actions kept after Algorithm 3.
    n_actions_removed : actions pruned.
...
class Algorithm3Output:
    ...
    [PAPER] "The important actions of C_{m|HD}^{kf}"

    Attributes
    ----------
    refined_actions     : List of retained ExecutiveActionEntry objects
                          (deep-copied, not mutating Algorithm 2 output).
    removed_actions     : List of pruned ExecutiveActionEntry objects
    ...
    def retained_for_node(self, node_id: str) -> List[ExecutiveActionEntry]:
        """All retained actions for a given node."""
    ...
    def retained_for_node_disease(
        ...
        """Retained actions for (node, disease class), sorted by weight desc."""
    ...
    def log_for_node(self, node_id: str) -> List[RefinementRecord]:
        """All refinement records for a given node."""
...
def validate_alg2_prerequisites(
    ...
    [INFER] Algorithm 3 depends entirely on Algorithm 2 outputs; running it
    without prior Algorithm 2 results for the node is invalid.
    ...
    # Check healthy ranges are finite
    ...
    # Check action weights are non-negative
...
def validate_fa0_constraint(retained: List[ExecutiveActionEntry]) -> List[str]:
    ...
    [PAPER] "FA = 0" means no false alarms for healthy class h=1.
    Operationally this means all retained actions have been built under the
    FA=0 policy (they were constructed in Algorithm 2 only for disease classes
    h >= 2).
...
def validate_output_consistency(
    ...
    Checks
    ------
    1. Every retained action was in Algorithm 2's library.
    2. No retained action's weight exceeds its Algorithm 2 weight.
    3. All removed actions have action_weight = 0 in the output.
    4. No duplicates in retained list for (node, feature, disease).
    ...
    # Check 1: every retained action exists in Algorithm 2
    seen_keys: set = set()
    ...
    # Check 3: removed actions have weight 0
...
def validate_newinf_monotonicity(log_records: List[RefinementRecord]) -> List[str]:
    ...
    [PAPER] newinf_u is only ever SET to 1, never reset to 0.  Therefore
    s = Σ newinf can only stay the same or increase.
...
def _get_raw_value(global_user_idx: int, feature_idx: int, data: np.ndarray) -> float:
    ...
    [ENGR]  In the UCI dataset, this is simply data[global_user_idx, feature_idx].
    Missing values (NaN) are handled by the caller (user is not flagged if NaN).
    """
...
def _is_outside_range(value: float, b_min: float, b_max: float) -> bool:
    ...
    Returns True if value is outside the healthy range, False otherwise.

    [ENGR]  NaN returns False – a missing measurement cannot be flagged as
    abnormal (we cannot conclude anything about the health of a user whose
...
def _refine_one_node(
    ...
    Parameters
    ----------
    node          : the TreeNode from Algorithm 1.
    ...
    Returns
    -------
    (retained, removed, log_records)
    """
    nid  = node.node_id
    m    = node.focus_level
    n    = len(node.user_indices)           # |U_m^k(f)|

    # ── Determine disease classes present in this node ─────────────────────
    # [PAPER] Line 1: "for h = 2 to H do"
    # [INFER] Only process disease classes that actually appear in this node
    #         AND have at least one action in the executive library.  Disease
    #         classes absent from the node cannot produce informative actions.
    ...
    # ── Initialise state [PAPER] ───────────────────────────────────────────
    # [PAPER] "buffer = 0, newinf = ∅"  – before BOTH loops.
    # newinf: boolean array aligned to node.user_indices (node-local indexing)
    # newinf[i] = True means node.user_indices[i] has been flagged as outside
    # the normal range by at least one (h, o) pair processed so far.
    newinf: np.ndarray = np.zeros(n, dtype=bool)   # shape (n,)
    buffer: int        = 0
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # OUTER LOOP: for h = 2 to H do   [PAPER line 1]
    # ─────────────────────────────────────────────────────────────────────────
    for h in disease_classes:
        ...
        # r_buf is a vector of action weights indexed by feature.
        h_actions: List[ExecutiveActionEntry] = [
            copy.copy(e)                      # [ENGR] copy to avoid mutating Alg2 output
            for e in alg2_output.executive_library
        ...
        if n_zero > 0:
            ...
            # Register zero-weight actions as removed (they carry no information)
            for a in h_actions:
                if a.action_weight == 0.0:
                    a_copy = copy.copy(a)
        ...
        # ─────────────────────────────────────────────────────────────────────
        # MIDDLE LOOP: for o = set O_m^k(f) do   [PAPER line 4]
        # [INFER] "set O_m^k(f_m^k)" here refers to the features available in
        #         this node that have non-zero action weight for disease h.
        #         We iterate in r_{o|h} descending order (established in line 3).
        # ─────────────────────────────────────────────────────────────────────
        for h_rank, action in enumerate(h_actions_sorted):
            o    = action.feature_idx
            o_name = action.feature_name

            # ── Retrieve perceptor model (healthy range) for (node, feature) ─
            ...
            # ─────────────────────────────────────────────────────────────────
            # INNER LOOP: for u = set of U_m^k(f) do   [PAPER line 5]
            # Check each user in the node against the healthy range.
            # [PAPER] Line 6-8:
            #   if BD_m^k(o, u) > b_max  OR  BD_m^k(o, u) < b_min:
            #       newinf_u = 1
            ...
            # ── Line 15: Buffer = s ──────────────────────────────────────────
            # [PAPER] Always update Buffer (even if feature was pruned).
            # [INFER] If s <= buffer (pruned), buffer stays at buffer (s == buffer).
            #         If s > buffer (retained), buffer increases to s.
            #         Since newinf is monotonically increasing, s >= buffer always.
            #         So Buffer = s effectively tracks the running maximum.
            buffer = s
...
def _compute_node_summary(
    ...
    [INFER] The summary aggregates per-disease and overall statistics for
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 7b – MINIMUM ACTION GUARANTEE
# ─────────────────────────────────────────────────────────────────────────────

def _guarantee_minimum_actions(
    retained:     List[ExecutiveActionEntry],
    alg2_output:  Algorithm2Output,
    node:         TreeNode,
) -> List[ExecutiveActionEntry]:
    """
    Ensure every disease class present in the node has at least 1 retained action.

    Algorithm 3's set-cover is greedy: if an early, high-coverage disease class
    (h=2) already flags every user that class h=6 would flag, h=6 gets zero
    retained actions and becomes undetectable.  This post-processing step
    restores the single highest-weight action for any such class, guaranteeing
    detectability while adding minimal noise.

    [PAPER] "the Executive keeps important actions for diagnosing between healthy
    and unhealthy situations" — zero actions defeats the intent entirely.
    """
    nid = node.node_id
    retained_by_h: Dict[int, List[ExecutiveActionEntry]] = {}
    for a in retained:
        if a.node_id == nid:
            retained_by_h.setdefault(a.disease_class, []).append(a)

    disease_classes = [
        h for h in node.health_dist
        if h != HEALTHY_CLASS and node.health_dist[h] > 0
    ]

    restored: List[ExecutiveActionEntry] = []
    for h in disease_classes:
        if len(retained_by_h.get(h, [])) == 0:
            # Find the best action from Algorithm 2 for this (node, disease) pair
            candidates = sorted(
                [e for e in alg2_output.executive_library
                 if e.node_id == nid and e.disease_class == h and e.action_weight > 0],
                key=lambda e: e.action_weight, reverse=True
            )
            if candidates:
                best = copy.copy(candidates[0])
                restored.append(best)
                log.debug(
                    f"  [min-action] node={nid!r} h={h}: restoring "
                    f"feat={best.feature_idx} r={best.action_weight:.4f}"
                )

    if restored:
        log.info(
            f"  [min-action] node={nid!r}: restored {len(restored)} actions "
            f"for classes {sorted(a.disease_class for a in restored)}"
        )
...
def run_algorithm3(
    ...
    Parameters
    ----------
    alg2_output  : Complete output of Algorithm 2 (perceptor + executive libraries).
    tree         : Decision tree from Algorithm 1.
    data         : Raw (N, 279) feature matrix.
    ...
    Returns
    -------
    Algorithm3Output with refined action library.

    Notes
    -----
    [PAPER]  "Algorithm 3 can refine actions in the executive training mode.
              This means that the executive keeps important actions for diagnosing
              between healthy and unhealthy situations."

    [ENGR]   We process nodes independently. Each node's newinf/buffer state is
             isolated from other nodes because each node has its own user set
             (U_m^{kf}) and its own feature set (O_m^{kf}).
    """
    log.info("=" * 70)
    ...
    # [INFER] We process exactly those nodes for which Algorithm 2 has produced
    #         executive actions.  If nodes_filter is given, we restrict further.
    ...
    for nid in nodes_to_process:
        ...
        # ── Restore at least 1 action per disease class ──────────────────────
        retained = _guarantee_minimum_actions(retained, alg2_output, node)
...
def compare_alg2_vs_alg3(
    ...
    for a given (node, disease class).  Shows which features were pruned and why.
    """
    sep = "─" * 78
    print(f"\n{sep}")
    ...
    # Get log records for this (node, h)
...
def analyse_coverage(
    ...
    For each retained action (node, feature, disease_h), computes:
      - n_diseased_flagged: diseased users of class h with value outside range
...
def assess_paper_fidelity(output: Algorithm3Output) -> None:
    ...
    [ENGR] This function documents every design decision, ambiguity, and
    potential mismatch with the paper's pseudocode.
    """
    sep = "=" * 72
    print(f"\n{sep}")
...
def main(
    ...
    # ── Step 1: Load dataset ──────────────────────────────────────────────────
    from CDS_Paper_Algorithms import load_dataset, build_decision_tree
    ...
    for nid in (nodes_filter or ["root"]):
        if nid not in alg3_output.node_summaries:
            continue
        # Detailed log for first few disease classes
        for h in [2, 5, 6]:
