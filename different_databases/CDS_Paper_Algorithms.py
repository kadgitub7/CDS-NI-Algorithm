...
# ─────────────────────────────────────────────────────────────────────────────
# DATASET-DRIVEN CONSTANTS
# These are populated from dataset_config at module level and can be updated
# by calling reload_config() after set_dataset().
# ─────────────────────────────────────────────────────────────────────────────
from dataset_config import get_config as _get_config

def _load_constants():
    cfg = _get_config()
    return (cfg.diagnostic_threshold, cfg.healthy_class, cfg.label_col_idx,
            cfg.n_features, cfg.feature_names, cfg.missing_value_cols)

_dt, _hc, _lci, _nf, _fn, _mvc = _load_constants()

DIAGNOSTIC_THRESHOLD: float = _dt
U_MIN: int = math.ceil(5 / DIAGNOSTIC_THRESHOLD)
COMPLEXITY_THRESHOLD: int = 16
HEALTHY_CLASS: int = _hc
LABEL_COL_IDX: int = _lci
N_FEATURES: int = _nf

def reload_config():
    """Re-read constants from dataset_config after a set_dataset() call."""
    global DIAGNOSTIC_THRESHOLD, U_MIN, HEALTHY_CLASS, LABEL_COL_IDX
    global N_FEATURES, FEATURE_NAMES, MISSING_VALUE_COLS
    cfg = _get_config()
    DIAGNOSTIC_THRESHOLD = cfg.diagnostic_threshold
    U_MIN = math.ceil(5 / DIAGNOSTIC_THRESHOLD)
    HEALTHY_CLASS = cfg.healthy_class
    LABEL_COL_IDX = cfg.label_col_idx
    N_FEATURES = cfg.n_features
    FEATURE_NAMES.clear()
    FEATURE_NAMES.update(cfg.feature_names)
    MISSING_VALUE_COLS = cfg.missing_value_cols
...
FEATURE_NAMES: Dict[int, str] = dict(_fn)
MISSING_VALUE_COLS: FrozenSet[int] = _mvc
...
def load_dataset(path: str = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load data using the active dataset configuration.

    For UCI: loads CSV with 279 features + 1 label column.
    For PhysioNet: extracts 40 features from raw ECG .mat files.

    Returns
    -------
    data   : np.ndarray, shape (N, n_features), float64, NaN for missing.
    labels : np.ndarray, shape (N,), int class labels.
    """
    cfg = _get_config()
    if path is None:
        path = cfg.data_path
    log.info(f"Loading dataset '{cfg.name}' from: {path}")
    data, labels = cfg.load_fn(path)
    n_feat = data.shape[1]
    log.info(f"  Loaded {data.shape[0]} users x {n_feat} features.")
    log.info(f"  Label distribution: "
             f"{ {c: int((labels==c).sum()) for c in sorted(set(labels))} }")
    missing_per_col = np.isnan(data).sum(axis=0)
    nonempty = [(i, int(missing_per_col[i])) for i in range(n_feat)
                if missing_per_col[i] > 0]
    if nonempty:
        log.info(f"  Columns with missing values: {dict(nonempty)}")
    return data, labels
...
def _fsd_binary(feature_idx: int, user_indices: np.ndarray,
    ...
    for bidx, v in enumerate(values_present, start=1):
        cfg = _get_config()
        if cfg.has_sex_feature and feature_idx == cfg.sex_feature_idx:
            lbl = "male" if v == cfg.male_value else "female"
        else:
            lbl = f"{fname}={int(v)}"
...
def main(data_path: str = None) -> DecisionTree:
    """
...
if __name__ == "__main__":
    ...
    dataset_name = sys.argv[1] if len(sys.argv) > 1 else "uci"
