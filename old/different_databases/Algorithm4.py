...
# Disease classes loaded from active dataset configuration.
from dataset_config import get_config as _get_cfg
_cfg_alg4 = _get_cfg()
ALL_DISEASE_CLASSES: Tuple[int, ...] = _cfg_alg4.disease_classes

# AF_NORMALISE_TO_CAPACITY: when True, rescales raw AF to [0,1] relative to
# the node's maximum achievable AF. This was intended to compensate for fewer
# disease classes producing less AF accumulation. However, with a single
# disease class and tight percentile-based healthy ranges, the normalization
# maps even tiny raw AF to 1.0, making every non-alarm user "healthy."
# Disabled: the raw AF combined with the diagnostic threshold and percentile-
# based ranges provides correct discrimination without artificial rescaling.
AF_NORMALISE_TO_CAPACITY: bool = False
AF_DISEASE_SCALE: float = 1.0

def reload_config_alg4():
    """Re-read dataset-specific constants after set_dataset()."""
    global ALL_DISEASE_CLASSES, AF_NORMALISE_TO_CAPACITY, _HAS_SEX, SEX_FEATURE_INDEX_ALG4
    cfg = _get_cfg()
    ALL_DISEASE_CLASSES = cfg.disease_classes
    AF_NORMALISE_TO_CAPACITY = False
    _HAS_SEX = cfg.has_sex_feature
    SEX_FEATURE_INDEX_ALG4 = cfg.sex_feature_idx
...
_HAS_SEX = _cfg_alg4.has_sex_feature
SEX_FEATURE_INDEX_ALG4: Optional[int] = _cfg_alg4.sex_feature_idx
...
class Algorithm4Output:
    ...
    def _recompute_stats(self) -> None:
        ...
        # Sex-based error analysis for abnormal users
        if self.data is not None and _HAS_SEX and SEX_FEATURE_INDEX_ALG4 is not None:
...
def _rl_select_best_action(
    ...
    if _fairness_cfg.ENABLE_FAIRNESS_RL and _HAS_SEX and data is not None and labels is not None:
    ...
    for h in disease_classes:
        ...
        while C_buf:
            ...
            selected_action, rl_entries = _rl_select_best_action(
                ...
                user_sex          = int(data[user_global_idx, SEX_FEATURE_INDEX_ALG4]) if _HAS_SEX else -1,
                data              = data,
                labels            = labels,
...
def run_loocv(
    ...
    for i in range(n_total):
        ...
        if _fc.ENABLE_FORCED_SEX_BRANCHING:
            ...
            test_user_sex = data[i, SEX_FEATURE_INDEX_ALG4] if _HAS_SEX else -1
            if test_user_sex == 0:
        ...
        if _fc.ENABLE_FORCED_SEX_BRANCHING:
            ...
            # Standard tree: if sex feature exists, use Sex branching at level 2.
            if _HAS_SEX and SEX_FEATURE_INDEX_ALG4 is not None:
...
if __name__ == "__main__":
    ...
    if not cfg.has_sex_feature:
        _fairness_cfg.ENABLE_FAIRNESS_RL = False
        _fairness_cfg.ENABLE_FORCED_SEX_BRANCHING = False
        _fairness_cfg.ENABLE_EQUALIZED_ODDS = False
        _fairness_cfg.ENABLE_DATA_AUGMENTATION = False
        _fairness_cfg.ENABLE_ADVERSARIAL_DEBIASING = False
        _fairness_cfg.ENABLE_REWEIGHING = False
    reload_config()
    reload_config_alg4()
    ...
    output = main(
        run_loocv_flag  = True,
        max_users       = n,
        run_tests       = (dataset_name == "uci"),
        fidelity        = (dataset_name == "uci"),
    )