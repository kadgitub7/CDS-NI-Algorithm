...
from dataset_config import get_config as _get_config
...
def _sex_feature_index():
    cfg = _get_config()
    return cfg.sex_feature_idx if cfg.has_sex_feature else None

SEX_FEATURE_INDEX = _sex_feature_index()
...
# Disease classes and healthy range percentile from active dataset config.
_cfg = _get_config()
ALL_DISEASE_CLASSES: Tuple[int, ...] = _cfg.disease_classes
ALL_CLASSES: Tuple[int, ...] = _cfg.all_classes
HEALTHY_RANGE_PERCENTILE = _cfg.healthy_range_percentile

def reload_config_alg2():
    """Re-read dataset-specific constants after set_dataset()."""
    global ALL_DISEASE_CLASSES, ALL_CLASSES, HEALTHY_RANGE_PERCENTILE, SEX_FEATURE_INDEX
    cfg = _get_config()
    ALL_DISEASE_CLASSES = cfg.disease_classes
    ALL_CLASSES = cfg.all_classes
    HEALTHY_RANGE_PERCENTILE = cfg.healthy_range_percentile
    SEX_FEATURE_INDEX = cfg.sex_feature_idx if cfg.has_sex_feature else None
...
def compute_executive_actions(disc:          DiscretizationResult,
    ...
    # [PAPER Line 19] r_{o|h} = P(B̂ < b_min|h) + P(B̂ > b_max|h)
    # b_min and b_max come from the healthy range (Eq.5 / percentile).
    # We find which bins correspond to values outside [b_min, b_max].
    # For large datasets with percentile-based ranges, this correctly
    # reflects the narrower healthy range rather than the full span of
    # all healthy users' bin assignments.
    global_valid  = node.user_indices[disc.valid_user_rows]
    labels_valid  = labels[global_valid]
    healthy_mask  = (labels_valid == HEALTHY_CLASS)
    ...
    # Map the percentile-based healthy range to bin indices.
    # A bin is "below normal" if its RIGHT edge <= b_min.
    # A bin is "above normal" if its LEFT edge >= b_max.
    min_healthy_bin = 0
    max_healthy_bin = disc.n_bins - 1
    for b_idx in range(disc.n_bins):
        if disc.bin_edges[b_idx + 1] <= b_min:
            min_healthy_bin = b_idx + 1
        else:
            break
    for b_idx in range(disc.n_bins - 1, -1, -1):
        if disc.bin_edges[b_idx] >= b_max:
            max_healthy_bin = b_idx - 1
        else:
            break
    min_healthy_bin = max(0, min_healthy_bin)
    max_healthy_bin = min(disc.n_bins - 1, max_healthy_bin)
    ...
    if ENABLE_REWEIGHING and data is not None and SEX_FEATURE_INDEX is not None:
...
def run_algorithm2_for_node(node:          TreeNode,
    ...
    for feature_o in node.feature_indices:
        ...
        if SEX_FEATURE_INDEX is not None:
...
if __name__ == "__main__":
    ...
    if not cfg.has_sex_feature:
        from fairness_config import ENABLE_REWEIGHING as _rw
        import fairness_config as _fc2
        _fc2.ENABLE_REWEIGHING = False
    reload_config()
    reload_config_alg2()
    full    = "--full" in _sys.argv
    output  = main(run_full=full)